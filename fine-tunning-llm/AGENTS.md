...
Este repositório implementa um pipeline em Python 3.12 para leitura de Excel, validação de qualidade, detecção de PHI/PII, anonimização, preparação de dados para fine-tuning e comparação com a inferência do modelo-base.

## Status Atual

Status revisado em 28/08/2026 a partir do código-fonte, sem leitura ou inspeção de `app/data`.
...
7. Inferência-base dos registros com IDs 2, 3 e 6 usando `Qwen/Qwen3-0.6B`, executada em CPU a partir do cache local do Hugging Face.

O menu de `main.py` apresenta essas funcionalidades como etapas 1 a 6; a identificação e a anonimização de PII fazem parte da etapa 4.

### Pendências conhecidas

- O treinamento do modelo ainda não está implementado. A etapa atual apenas prepara o dataset e gera a inferência-base para comparação futura.
- As dependências `peft`, `trl`, `accelerate`, `datasets` e `evaluate` já estão declaradas, mas ainda não são usadas por uma rotina de treinamento.
...
- `main.py`: ponto de entrada local, menu e orquestração das seis etapas do pipeline.
...
- `app/services/fine_tuning_service.py`: preparação do dataset de conversas e inferência do modelo-base Qwen.
